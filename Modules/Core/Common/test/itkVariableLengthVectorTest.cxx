...
int itkVariableLengthVectorTest(int, char*[])
{
  ...
  d[0] = 0.1; d[1] = 0.2; d[2] = 0.3;
    ...
    {
    DoubleVariableLengthVectorType x( d, 3, false );
    if( (!itk::Math::FloatAlmostEqual(d[0] , 0.1)) || (x[0] != 0.1) )
      {
    ...
    x[4] = 4.0;
    std::cout << x << std::endl;
    if( (!itk::Math::FloatAlmostEqual(d[0] , 0.1)) || (x[0] != 0.1) ) // increase length but preserve existing data
      {
      std::cerr << "Memory management: [FAILED]" << std::endl;
      }
    x.SetSize( 2 , false); // reduce length but preserve existing data
    std::cout << x << std::endl;
    if( (x.GetSize() != 2) || (!itk::Math::FloatAlmostEqual(d[0] , 0.1)) || (x[0] != 0.1) )
      {
      std::cerr << "Memory management: [FAILED]" << std::endl;
      }
