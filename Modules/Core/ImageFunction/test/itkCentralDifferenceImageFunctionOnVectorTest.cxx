...
int itkCentralDifferenceImageFunctionOnVectorTestRun( )
{
  ...
  for( itk::SizeValueType n=0; n < VectorLength; n++ )
    {
    if( !itk::Math::FloatAlmostEqual(indexOutput(n,0) , itk::NumericTraits<OutputValueType>::ZeroValue()) )
      {
  ...
  for( itk::SizeValueType n=0; n < VectorLength; n++ )
    {
    if( !itk::Math::FloatAlmostEqual(indexOutput(n,1) , itk::NumericTraits<OutputValueType>::ZeroValue()) )
      {
  ...
  if( !itk::Math::FloatAlmostEqual(directionOnDerivative[0][0] , -origDerivative[0][0]) ||
      !itk::Math::FloatAlmostEqual(directionOnDerivative[0][1] , -origDerivative[0][1]) )
    {
