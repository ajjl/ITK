...
::MetaObjectToSpatialObject(const MetaObjectType *mo)
{
  ...
  for ( unsigned int identifier = 0; identifier < tube->GetPoints().size(); identifier++ )
    ...
    if ( !itk::Math::FloatAlmostEqual(( *it2 )->GetField("r") , -1) )
      {
      pnt.SetRadius( ( *it2 )->GetField("r") );
      }

    char vnd[] = "v1x";
    if ( !itk::Math::FloatAlmostEqual(( *it2 )->GetField( vnd ) , -1) )
      {
    ...
    char td[] = "tx";
    if ( !itk::Math::FloatAlmostEqual(( *it2 )->GetField( td ) , -1) )
      {
      t[0] = ( *it2 )->GetField( td );
      for( unsigned int ii = 1; ii < ndims; ++ii )
        {
    ...
    if ( !itk::Math::FloatAlmostEqual(( *it2 )->GetField("red") , -1) )
      {
      pnt.SetRed( ( *it2 )->GetField("red") );
      }

    if ( !itk::Math::FloatAlmostEqual(( *it2 )->GetField("green") , -1) )
      {
      pnt.SetGreen( ( *it2 )->GetField("green") );
      }

    if ( !itk::Math::FloatAlmostEqual(( *it2 )->GetField("blue") , -1) )
      {
      pnt.SetBlue( ( *it2 )->GetField("blue") );
      }

    if ( !itk::Math::FloatAlmostEqual(( *it2 )->GetField("alpha") , -1) )
      {
      pnt.SetAlpha( ( *it2 )->GetField("alpha") );
      }

    if ( !itk::Math::FloatAlmostEqual(( *it2 )->GetField("id") , -1) )
      {
...
::SpatialObjectToMetaObject(const SpatialObjectType *spatialObject)
{
  ...
  for ( it = DTITubeSO->GetPoints().begin(); it != DTITubeSO->GetPoints().end(); ++it )
    ...
    if ( !itk::Math::FloatAlmostEqual(( *it ).GetRadius() , 0) )
      {
      writeRadius = true;
      }
    ...
    for ( d = 0; d < NDimensions; d++ )
      {
      if ( !itk::Math::FloatAlmostEqual(( *it ).GetNormal1()[d] , 0) )
        {
        writeNormal1 = true;
        }
      if ( !itk::Math::FloatAlmostEqual(( *it ).GetNormal2()[d] , 0) )
        {
        writeNormal2 = true;
        }
      if ( !itk::Math::FloatAlmostEqual(( *it ).GetTangent()[d] , 0) )
        {
        writeTangent = true;
        }
      }
