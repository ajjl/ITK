...
int itkBlobSpatialObjectTest(int, char* [])
{
  ...
  while(it != blob->GetPoints().end())
  {
    ...
    {
      if(!itk::Math::FloatAlmostEqual((*it).GetPosition()[d] , i+d))
      {
        std::cout<<"[FAILED]"<<std::endl;
        return EXIT_FAILURE;
      }
  ...
  while(it != blob->GetPoints().end())
  {
    for(unsigned int d=0;d<3;d++)
    {
      if(!itk::Math::FloatAlmostEqual((*it).GetBlue() , i))
      {
        std::cout<<"[FAILED]"<<std::endl;
        return EXIT_FAILURE;
      }
      if(!itk::Math::FloatAlmostEqual((*it).GetGreen() , i+1))
      {
        std::cout<<"[FAILED]"<<std::endl;
        return EXIT_FAILURE;
      }

      if(!itk::Math::FloatAlmostEqual((*it).GetRed() , i+2))
      {
        std::cout<<"[FAILED]"<<std::endl;
        return EXIT_FAILURE;
      }

      if(!itk::Math::FloatAlmostEqual((*it).GetAlpha() , i+3))
      {
        std::cout<<"[FAILED]"<<std::endl;
        return EXIT_FAILURE;
      }
  ...
  if(!itk::Math::FloatAlmostEqual(value , 1))
  {
     std::cout<<"[FAILED]"<<std::endl;
     return EXIT_FAILURE;
  }
