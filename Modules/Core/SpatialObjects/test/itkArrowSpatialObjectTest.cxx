...
int itkArrowSpatialObjectTest(int, char* [])
{
  ...
  myArrow->SetLength(2);
  if(! itk::Math::FloatAlmostEqual(myArrow->GetLength(), 2.0))
    {
  ...
  if(  (! itk::Math::FloatAlmostEqual(myArrow->GetDirection()[0], 0.0))
    || (! itk::Math::FloatAlmostEqual(myArrow->GetDirection()[1], 1.0))
    || (! itk::Math::FloatAlmostEqual(myArrow->GetDirection()[2], 0.0))
    )
    {
    std::cout << "[FAILURE]" << std::endl;
    return EXIT_FAILURE;
  ...
  if(  (! itk::Math::FloatAlmostEqual(boundingBox->GetBounds()[2], 0.0))
    || (! itk::Math::FloatAlmostEqual(boundingBox->GetBounds()[3], 1.0))
    )
    {
      std::cout<<"[FAILED]"<<std::endl;
      return EXIT_FAILURE;
