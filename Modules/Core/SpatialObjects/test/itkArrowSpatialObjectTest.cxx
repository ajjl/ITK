...
int itkArrowSpatialObjectTest(int, char* [])
{
  ...
  myArrow->SetLength(2);
  if(!itk::Math::FloatAlmostEqual(myArrow->GetLength() , 2))
    {
  ...
  if(!itk::Math::FloatAlmostEqual(myArrow->GetDirection()[0] , 0)
    || !itk::Math::FloatAlmostEqual(myArrow->GetDirection()[1] , 1)
    || !itk::Math::FloatAlmostEqual(myArrow->GetDirection()[2] , 0)
    )
    {
    std::cout << "[FAILURE]" << std::endl;
    return EXIT_FAILURE;
  ...
  if( (!itk::Math::FloatAlmostEqual(boundingBox->GetBounds()[2] , 0) )
     || (!itk::Math::FloatAlmostEqual(boundingBox->GetBounds()[3] , 1) )
      )
    {
      std::cout<<"[FAILED]"<<std::endl;
      return EXIT_FAILURE;
