...
int itkEllipseSpatialObjectTest(int, char* [])
{
  ...
  for(unsigned int i = 0; i<4;i++)
  {
    if(itk::Math::NotEqualsComparison(radius2[i],i))
    {
 ...
 EllipseType::ArrayType radius3 = myEllipse->GetRadius();
  ...
  for(unsigned int i = 0; i<4;i++)
  {
    if(itk::Math::NotEqualsComparison(radius3[i],3))
    {
      std::cout << "[FAILURE]" << std::endl;
      return EXIT_FAILURE;
    }
  ...
  if( (itk::Math::NotEqualsComparison(offset3[0],5)) || (itk::Math::NotEqualsComparison(offset3[1],5))
     ||(itk::Math::NotEqualsComparison(offset3[2],5)) ||(itk::Math::NotEqualsComparison(offset3[3],5))
     )
  {
    std::cout<<"[FAILED]"<<std::endl;
    return EXIT_FAILURE;
