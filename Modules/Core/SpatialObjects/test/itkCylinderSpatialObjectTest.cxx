...
int itkCylinderSpatialObjectTest(int, char* [])
{
  ...
  if(!itk::Math::FloatAlmostEqual(radius2 , radius))
    {
  ...
  if( (!itk::Math::FloatAlmostEqual(boundingBox->GetBounds()[0] , -3) )
    || (!itk::Math::FloatAlmostEqual(boundingBox->GetBounds()[1] , 3) )
    || (!itk::Math::FloatAlmostEqual(boundingBox->GetBounds()[2] , -6) )
    || (!itk::Math::FloatAlmostEqual(boundingBox->GetBounds()[3] , 6) )
    || (!itk::Math::FloatAlmostEqual(boundingBox->GetBounds()[4] , -3) )
    || (!itk::Math::FloatAlmostEqual(boundingBox->GetBounds()[5] , 3) )
    )
   {
   std::cout<<"[FAILED]"<<std::endl;
   return EXIT_FAILURE;
