  ...
  const unsigned int componentToExtract )
{
  ...
  if(   (!itk::Math::FloatAlmostEqual(vectorImageAdaptor->GetPixel(index) , vectorImage->GetPixel( index )[componentToExtract]))
     || (!itk::Math::FloatAlmostEqual(vectorImage->GetPixel( index )[componentToExtract] , componentToExtract) ))
    {
  ...
  while (!adaptIt.IsAtEnd())
    ...
    if (!itk::Math::FloatAlmostEqual(pixel , f[componentToExtract]))
      {
      itFailed = true;
...
int itkVectorImageTest( int, char* argv[] )
{
  ...
  while( !cit.IsAtEnd() )
    ...
    for (unsigned int i = 0; i < Dimension; i++)
      {
      if (!itk::Math::FloatAlmostEqual(cit.Get()[i] , cit.GetIndex()[i]) ||
          !itk::Math::FloatAlmostEqual(cit.Get()[i+Dimension] , cit.GetIndex()[i]))
        {
        failed1 = true;
        }
      }
  ...
  if( failed1 )
    ...
    if( (!itk::Math::FloatAlmostEqual(neighborhood[0][0] , 0)) || (neighborhood[0][2*Dimension-1] != (Dimension-1)))
      {
    ...
    if (Dimension == 3)
      ...
      while (! ci.IsAtEnd())
        ...
        if( (offset2[0] == -1) && (offset2[1]== -1) && (offset2[2]== -1) )
          ...
          if( (!itk::Math::FloatAlmostEqual(ci.Get()[0] , 0)) || (ci.Get()[1] != 1) || (ci.Get()[2] != 2) )
            {
            failed=true;
            std::cerr
