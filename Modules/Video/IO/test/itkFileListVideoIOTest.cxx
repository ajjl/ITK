                           ...
                           double inFpS )
{
  ...
  for (SizeValueType i = 0; i < fileListIO->GetFrameTotal(); ++i)
    ...
    for (unsigned int j = 0; j < ImageType::ImageDimension; ++j)
      {
      if (!itk::Math::FloatAlmostEqual(fileListIO->GetSpacing(j) , reader->GetImageIO()->GetSpacing(j)))
        {
        std::cerr << "Spacing not read correctly" << std::endl;
        ret = false;
        }
      if (!itk::Math::FloatAlmostEqual(fileListIO->GetOrigin(j) , reader->GetImageIO()->GetOrigin(j)))
        {
  ...
  if (!itk::Math::FloatAlmostEqual(fileListIO->GetFramesPerSecond() , fps) || fileListIO->GetDimensions(0) != width ||
