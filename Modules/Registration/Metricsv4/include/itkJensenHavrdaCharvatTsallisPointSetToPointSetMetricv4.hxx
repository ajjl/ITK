...
::ComputeValueAndDerivative( const PointType & samplePoint, MeasureType & value, LocalDerivativeType & derivativeReturn, bool calcValue, bool calcDerivative ) const
{
  ...
  if( Math::EqualsComparison( probabilityStar, NumericTraits<RealType>::ZeroValue() ) )
    {
    return;
    }
  ...
  if( calcValue )
    ...
    if( Math::EqualsComparison( this->m_Alpha, realOne ) )
      {
  ...
  if( calcDerivative )
    ...
    for( SizeValueType n = 0; n < neighbors.size(); n++ )
      ...
      if( Math::EqualsComparison( gaussian, NumericTraits<RealType>::ZeroValue() ) )
        {
        continue;
        }
