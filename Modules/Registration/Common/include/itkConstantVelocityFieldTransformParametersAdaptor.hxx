...
::SetRequiredSize( const SizeType & size )
{
  ...
  for( SizeValueType d = 0; d < ConstantVelocityFieldDimension; d++ )
    {
    if( !itk::Math::FloatAlmostEqual(this->m_RequiredFixedParameters[d] , size[d]) )
      {
      isModified = true;
      }
...
::SetRequiredOrigin( const PointType & origin )
{
  ...
  for( SizeValueType d = 0; d < ConstantVelocityFieldDimension; d++ )
    {
    if( !itk::Math::FloatAlmostEqual(this->m_RequiredFixedParameters[ConstantVelocityFieldDimension + d] , origin[d]) )
      {
      isModified = true;
      }
...
::SetRequiredSpacing( const SpacingType & spacing )
{
  ...
  for( SizeValueType d = 0; d < ConstantVelocityFieldDimension; d++ )
    {
    if( !itk::Math::FloatAlmostEqual(this->m_RequiredFixedParameters[2*ConstantVelocityFieldDimension + d] , spacing[d]) )
      {
      isModified = true;
      }
...
::SetRequiredDirection( const DirectionType & direction )
{
  ...
  for( SizeValueType di = 0; di < ConstantVelocityFieldDimension; di++ )
    ...
    for( SizeValueType dj = 0; dj < ConstantVelocityFieldDimension; dj++ )
      {
      if( !itk::Math::FloatAlmostEqual(this->m_RequiredFixedParameters[3 * ConstantVelocityFieldDimension + ( di * ConstantVelocityFieldDimension + dj )] , direction[di][dj]) )
        {
        isModified = true;
        }
