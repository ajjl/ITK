...
::GetValue(const TransformParametersType & parameters) const
{
  ...
  while( !fi.IsAtEnd() )
    ...
    //
    //
    if( itk::Math::FloatAlmostEqual(fixedValue , m_ForegroundValue) )
      {
      fixedForegroundArea++;
      }
    ...
    if( this->m_Interpolator->IsInsideBuffer(transformedPoint) )
      ...
      if( itk::Math::FloatAlmostEqual(movingValue , m_ForegroundValue) )
        {
        movingForegroundArea++;
        }
      if( ( itk::Math::FloatAlmostEqual(movingValue , m_ForegroundValue) ) && ( fixedValue == m_ForegroundValue ) )
        {
        intersection++;
        }
      }
  ...
  while( !mit.IsAtEnd() )
    ...
    for( unsigned int i = 0; i < dim; i++ )
      ...
      if( ( currIndex[i] == 0 )
        ...
        if( ( !itk::Math::FloatAlmostEqual(minusVal , m_ForegroundValue) ) && ( plusVal == m_ForegroundValue ) )
          {
          tempGradPixel[i] = 1;
          }
        else if( ( !itk::Math::FloatAlmostEqual(minusVal , m_ForegroundValue) ) && ( plusVal != m_ForegroundValue ) )
          {
          tempGradPixel[i] = -1;
          }
        else
