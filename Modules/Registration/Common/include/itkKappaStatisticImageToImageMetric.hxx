...
::GetValue(const TransformParametersType & parameters) const
{
  ...
  while( !fi.IsAtEnd() )
    ...
    //
    //
    if( Math::EqualsComparison(fixedValue, m_ForegroundValue) )
      {
      fixedForegroundArea++;
      }
    ...
    if( this->m_Interpolator->IsInsideBuffer(transformedPoint) )
      ...
      if( Math::EqualsComparison(movingValue, m_ForegroundValue) )
        {
        movingForegroundArea++;
        }
      if( ( Math::EqualsComparison(movingValue, m_ForegroundValue) ) && ( Math::EqualsComparison(fixedValue, m_ForegroundValue) ) )
        {
        intersection++;
        }
      }
  ...
  while( !mit.IsAtEnd() )
    ...
    for( unsigned int i = 0; i < dim; i++ )
      ...
      if( ( currIndex[i] == 0 )
        ...
        if( ( Math::NotEqualsComparison(minusVal, m_ForegroundValue) ) && ( Math::EqualsComparison(plusVal, m_ForegroundValue) ) )
          {
          tempGradPixel[i] = 1;
          }
