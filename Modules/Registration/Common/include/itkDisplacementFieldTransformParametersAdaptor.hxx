...
::SetRequiredOrigin( const PointType & origin )
{
  ...
  for( SizeValueType d = 0; d < SpaceDimension; d++ )
    {
    if( !itk::Math::FloatAlmostEqual(this->m_RequiredFixedParameters[SpaceDimension + d] , origin[d]) )
      {
      isModified = true;
      }
...
::SetRequiredSpacing( const SpacingType & spacing )
{
  ...
  for( SizeValueType d = 0; d < SpaceDimension; d++ )
    {
    if( !itk::Math::FloatAlmostEqual(this->m_RequiredFixedParameters[2*SpaceDimension + d] , spacing[d]) )
      {
      isModified = true;
      }
...
::SetRequiredDirection( const DirectionType & direction )
{
  ...
  for( SizeValueType di = 0; di < SpaceDimension; di++ )
    ...
    for( SizeValueType dj = 0; dj < SpaceDimension; dj++ )
      {
      if( !itk::Math::FloatAlmostEqual(this->m_RequiredFixedParameters[3 * SpaceDimension + ( di * SpaceDimension + dj )] , direction[di][dj]) )
        {
        isModified = true;
        }
