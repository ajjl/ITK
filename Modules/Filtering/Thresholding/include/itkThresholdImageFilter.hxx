...
{
  if ( !itk::Math::FloatAlmostEqual(m_Upper , thresh)
       || m_Lower > NumericTraits< PixelType >::NonpositiveMin() )
    {
    m_Lower = NumericTraits< PixelType >::NonpositiveMin();
    m_Upper = thresh;
...
{
  if ( !itk::Math::FloatAlmostEqual(m_Lower , thresh) || m_Upper < NumericTraits< PixelType >::max() )
    {
    m_Lower = thresh;
    m_Upper = NumericTraits< PixelType >::max();
    this->Modified();
...
::ThresholdOutside(const PixelType & lower, const PixelType & upper)
{
  ...
  if ( !itk::Math::FloatAlmostEqual(m_Lower , lower) || m_Upper != upper )
    {
    m_Lower = lower;
    m_Upper = upper;
    this->Modified();
