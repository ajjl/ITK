                       ...
                       ThreadIdType threadId)
{
  ...
  if ( m_CurrentDimension == ImageDimension - 1 && !this->m_SquaredDistance )
    ...
    while ( !Ot.IsAtEnd() )
      ...
      if ( !itk::Math::FloatAlmostEqual(It.Get() , this->m_BackgroundValue) )
        {
...
::Voronoi(unsigned int d, OutputIndexType idx, OutputImageType *output)
{
  ...
  for ( unsigned int i = 0; i < nd; i++ )
    ...
    if ( !itk::Math::FloatAlmostEqual(di , NumericTraits< OutputPixelType >::max()) )
      {
      if ( l < 1 )
        {
        l++;
  ...
  for ( unsigned int i = 0; i < nd; i++ )
    ...
    if ( !itk::Math::FloatAlmostEqual(m_InputCache->GetPixel(idx) , this->m_BackgroundValue) )
      {
