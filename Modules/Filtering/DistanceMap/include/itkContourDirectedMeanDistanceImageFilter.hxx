                       ...
                       ThreadIdType threadId)
{
  ...
  for ( typename FaceListType::iterator fit = faceList.begin(); fit != faceList.end(); ++fit )
    ...
    while ( !bit.IsAtEnd() )
      ...
      if ( Math::NotEqualsComparison(bit.GetCenterPixel(), NumericTraits< InputImage1PixelType >::ZeroValue()) )
        {
        ...
        for ( unsigned int i = 0; i < neighborhoodSize; ++i )
          ...
          if ( Math::EqualsComparison(bit.GetPixel(i), NumericTraits< InputImage1PixelType >::ZeroValue()) )
            {
            bIsOnContour = true;
            break;
            }
