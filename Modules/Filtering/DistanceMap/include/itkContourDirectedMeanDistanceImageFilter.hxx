                       ...
                       ThreadIdType threadId)
{
  ...
  for ( typename FaceListType::iterator fit = faceList.begin(); fit != faceList.end(); ++fit )
    ...
    while ( !bit.IsAtEnd() )
      ...
      if ( !itk::Math::FloatAlmostEqual(bit.GetCenterPixel() , NumericTraits< InputImage1PixelType >::ZeroValue()) )
        {
        ...
        for ( unsigned int i = 0; i < neighborhoodSize; ++i )
          ...
          if ( itk::Math::FloatAlmostEqual(bit.GetPixel(i) , NumericTraits< InputImage1PixelType >::ZeroValue()) )
            {
            bIsOnContour = true;
            break;
            }
