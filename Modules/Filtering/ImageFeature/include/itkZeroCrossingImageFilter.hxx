    ...
    while ( !bit.IsAtEnd() )
      ...
      for ( i = 0; i < ImageDimension * 2; i++ )
        ...
        if ( ( ( this_one < zero ) && ( that > zero ) )
             ...
             || ( ( itk::Math::NotEqualsComparison(this_one , zero) ) && ( itk::Math::EqualsComparisonCaller(that , zero) ) ) )
          {
          ...
          else if ( itk::Math::EqualsComparisonCaller(abs_this_one , abs_that) && i >= ImageDimension )
            {
            it.Set(m_ForegroundValue);
            break;
            }
