...
int itkGaussianImageSourceTest(int, char* [] )
{
  ...
  if ( !itk::Math::FloatAlmostEqual(params[0] , sigma[0]) || params[1] != sigma[1] || params[2] != sigma[2] )
    {
    std::cerr << "Parameters have incorrect sigma value." << std::endl;
    return EXIT_FAILURE;
    }

  if ( !itk::Math::FloatAlmostEqual(params[3] , mean[0]) || params[4] != mean[1] || params[5] != mean[2] )
    {
    std::cerr << "Parameters have incorrect mean value." << std::endl;
    return EXIT_FAILURE;
    }

  if ( !itk::Math::FloatAlmostEqual(params[6] , source->GetScale()) )
    {
  ...
  if ( !itk::Math::FloatAlmostEqual(source->GetSigma()[0] , params[0]) ||
       !itk::Math::FloatAlmostEqual(source->GetSigma()[1] , params[1]) ||
       !itk::Math::FloatAlmostEqual(source->GetSigma()[2] , params[2]) )
    {
  ...
  if ( !itk::Math::FloatAlmostEqual(source->GetMean()[0] , params[3]) ||
       !itk::Math::FloatAlmostEqual(source->GetMean()[1] , params[4]) ||
       !itk::Math::FloatAlmostEqual(source->GetMean()[2] , params[5]) )
    {
  ...
  if ( !itk::Math::FloatAlmostEqual(source->GetScale() , params[6]) )
    {
