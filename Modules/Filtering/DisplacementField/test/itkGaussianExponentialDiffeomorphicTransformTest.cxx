...
int itkGaussianExponentialDiffeomorphicTransformTest(int ,char *[] )
{
  ...
  for( unsigned int i=0; i < displacementTransform->GetNumberOfParameters();
    ...
    {
    bool ok = true;
    if( i < linelength && !itk::Math::FloatAlmostEqual(params[i] , 0) )
      {
      ok = false;
      std::cout << params[i] << " != 0 " << std::endl;
      }
    if( i % linelength == 0 && !itk::Math::FloatAlmostEqual(params[i] , 0) )
      {
      ok = false;
      std::cout << params[i] << " != 0 " << std::endl;
      }
    if( i % linelength == (linelength - 1) && !itk::Math::FloatAlmostEqual(params[i] , 0) )
      {
      ok = false;
      std::cout << params[i] << " != 0 " << std::endl;
      }
  ...
  for(int i=-2; i< 3; i++ )
    {
     for(int j=-2; j< 3; j++ )
      ...
      unsigned int index = outlier +
        (unsigned int) (i * (signed int)(dimLength*dimensions) + j);
      std::cout << params(index) << " ";
      if( itk::Math::FloatAlmostEqual(params(index) , paramsFillValue) )
        {
