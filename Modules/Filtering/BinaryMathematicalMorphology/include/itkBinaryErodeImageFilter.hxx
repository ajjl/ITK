...
::GenerateData()
{
  ...
  for ( iRegIt.GoToBegin(), tmpRegIt.GoToBegin();
    ...
    if ( !itk::Math::FloatAlmostEqual(pxl , foregroundValue) )
      {
      tmpRegIt.Set(onTag);
      }
    else
    ...
    while ( !ouRegIndexIt.IsAtEnd() )
      ...
      for ( vecIt = vecBeginIt; vecIt != vecEndIt; ++vecIt )
        ...
        if ( !inputRegionForThread.IsInside(translatedIndex)
             || !itk::Math::FloatAlmostEqual(input->GetPixel(translatedIndex) , foregroundValue) )
          {
  ...
  for ( inIt.GoToBegin(), outIt.GoToBegin(); !outIt.IsAtEnd(); ++outIt, ++inIt )
    ...
    if ( !itk::Math::FloatAlmostEqual(outValue , backgroundValue) && inValue != foregroundValue )
      {
