...
::GenerateData()
{
  ...
  for ( iRegIt.GoToBegin(), tmpRegIt.GoToBegin();
    ...
    if ( Math::NotEqualsComparison(pxl, foregroundValue) )
      {
      tmpRegIt.Set(onTag);
      }
    else
    ...
    while ( !ouRegIndexIt.IsAtEnd() )
      ...
      for ( vecIt = vecBeginIt; vecIt != vecEndIt; ++vecIt )
        ...
        if ( !inputRegionForThread.IsInside(translatedIndex)
             || Math::NotEqualsComparison(input->GetPixel(translatedIndex), foregroundValue) )
          {
  ...
  for ( inIt.GoToBegin(), outIt.GoToBegin(); !outIt.IsAtEnd(); ++outIt, ++inIt )
    ...
    if ( Math::NotEqualsComparison(outValue, backgroundValue) && Math::NotEqualsComparison(inValue, foregroundValue) )
      {
