...
{
  if ( itk::Math::FloatAlmostEqual(m_ObjectValue , 0) )
    {
                       ...
                       ThreadIdType threadId)
{
  ...
  while ( !oRegIter.IsAtEnd() )
    {
    if ( !itk::Math::FloatAlmostEqual(oRegIter.Get() , m_ObjectValue) )
      {
  ...
  for ( fit = faceList.begin(); fit != faceList.end(); ++fit )
    ...
    while ( !iSNIter.IsAtEnd() )
      {
      if ( itk::Math::FloatAlmostEqual(iSNIter.GetCenterPixel() , m_ObjectValue) )
        {
...
::IsObjectPixelOnBoundary(const InputNeighborhoodIteratorType & iNIter)
{
  ...
  if ( m_UseBoundaryCondition )
    ...
    for ( i = 0; i < s; i++ )
      ...
      if ( !itk::Math::FloatAlmostEqual(tf , m_ObjectValue) )
        {
        return true;
        }
      }
    ...
    for ( i = 0; i < s; i++ )
      ...
      if ( !itk::Math::FloatAlmostEqual(tf , m_ObjectValue) && isInside )
        {
        return true;
        }
      }
