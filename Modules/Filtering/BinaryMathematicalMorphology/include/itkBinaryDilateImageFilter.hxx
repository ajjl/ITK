...
::GenerateData()
{
  ...
  for ( inIt.GoToBegin(), outIt.GoToBegin(); !outIt.IsAtEnd(); ++outIt, ++inIt )
    ...
    if ( itk::Math::EqualsComparisonCaller(value , foregroundValue) )
      {
  ...
  for ( iRegIt.GoToBegin(), tmpRegIt.GoToBegin();
    ...
    if ( itk::Math::EqualsComparisonCaller(pxl , foregroundValue) )
      {
      tmpRegIt.Set(onTag);
      }
    else
    ...
    while ( !ouRegIndexIt.IsAtEnd() )
      ...
      for ( vecIt = vecBeginIt; vecIt != vecEndIt; ++vecIt )
        ...
        if ( !inputRegionForThread.IsInside(translatedIndex)
             || itk::Math::EqualsComparisonCaller(input->GetPixel(translatedIndex) , foregroundValue) )
          {
