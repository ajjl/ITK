...
::ComputeInternalForce(SimplexMeshGeometry *data)
{
  ...
  if ( !itk::Math::FloatAlmostEqual(L_Ref , (double)NumericTraits< IdentifierType >::max()) && L != (double)NumericTraits< IdentifierType >::max() )
    {
...
::L_Func(double r, double d, double phi)
{
  ...
  if ( tmpSqr > 0 )
    ...
    if ( !itk::Math::FloatAlmostEqual(denom , 0) )
      {
