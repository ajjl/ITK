...
::GetPixelIndexFromPolygon(PointTypeDeque vertlist, IndexList *PixelPool)
{
  ...
  if ( itk::Math::EqualsComparisonCaller(leftP[1] , beginy) )
    {
    leftDx = 1.0;
    }
  else
  ...
  if ( itk::Math::EqualsComparisonCaller(rightP[1] , beginy) )
    {
    rightDx = 1.0;
    }
  else
  ...
  while ( vsize > 2 )
    ...
    if ( RorL )
      ...
      rightP = vertlist.back();
      if ( itk::Math::EqualsComparisonCaller(rightP[1] , currP[1]) )
        {
        rightDx = 1.0;
        }
      else
      ...
      leftP = vertlist.front();
      if ( itk::Math::EqualsComparisonCaller(leftP[1] , currP[1]) )
        {
        leftDx = 1.0;
        }
      else
  ...
  if ( intbeginy <= intendy )
    ...
    if ( RorL )
      {
      if ( itk::Math::EqualsComparisonCaller(rightP[1] , leftP[1]) )
        {
        rightDx = 1.0;
        }
      else
