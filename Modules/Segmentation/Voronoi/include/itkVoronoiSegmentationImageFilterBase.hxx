...
::GetPixelIndexFromPolygon(PointTypeDeque vertlist, IndexList *PixelPool)
{
  ...
  if ( Math::EqualsComparison(leftP[1], beginy) )
    {
    leftDx = 1.0;
    }
  else
  ...
  if ( Math::EqualsComparison(rightP[1], beginy) )
    {
    rightDx = 1.0;
    }
  else
  ...
  while ( vsize > 2 )
    ...
    if ( RorL )
      ...
      rightP = vertlist.back();
      if ( Math::EqualsComparison(rightP[1], currP[1]) )
        {
        rightDx = 1.0;
        }
      else
      ...
      leftP = vertlist.front();
      if ( Math::EqualsComparison(leftP[1], currP[1]) )
        {
        leftDx = 1.0;
        }
      else
  ...
  if ( intbeginy <= intendy )
    ...
    if ( RorL )
      {
      if ( Math::EqualsComparison(rightP[1], leftP[1]) )
        {
        rightDx = 1.0;
        }
      else
