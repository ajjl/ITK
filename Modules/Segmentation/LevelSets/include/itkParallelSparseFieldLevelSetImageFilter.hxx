...
::ConstructActiveLayer()
{
  ...
  for ( outputIt.GoToBegin(); !outputIt.IsAtEnd(); ++outputIt )
    {
    bounds_status = true;
    if ( itk::Math::FloatAlmostEqual(outputIt.GetCenterPixel() , m_ValueZero) )
      {
      ...
      if ( bounds_status == true )
        ...
        for ( unsigned int i = 0; i < m_NeighborList.GetSize(); ++i )
          ...
          if ( !itk::Math::FloatAlmostEqual(outputIt.GetPixel( m_NeighborList.GetArrayIndex(i) ) , m_ValueZero)
...
::ThreadedCalculateChange(ThreadIdType ThreadId)
{
  ...
  for (; layerIt != layerEnd; ++layerIt )
    ...
    if ( this->m_InterpolateSurfaceLocation
         && !itk::Math::FloatAlmostEqual(( centerValue = outputIt.GetCenterPixel() ) , NumericTraits< ValueType >::ZeroValue()) )
      {
