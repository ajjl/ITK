  ...
  for( int e = 0; e < numElements;  e++ )
    ...
    for( int j = 0; j < Ne; j++ )
      ...
      for( int k = 0; k < Ne; k++ )
         ...
         */
        if( !itk::Math::FloatAlmostEqual(Ke(j, k) , Float(0.0)) || Me(j, k) != Float(0.0) )
          {
    ...
    if( LoadLandmark::Pointer l3 = dynamic_cast<LoadLandmark *>( this->m_FEMObject->GetLoad(l2).GetPointer() ) )
      ...
      for( int j = 0; j < Ne; j++ )
        ...
        for( int k = 0; k < Ne; k++ )
          ...
          if( !itk::Math::FloatAlmostEqual(Le(j, k) , Float(0.0)) )
            {
            // lhs matrix
...
::BrentsMethod(Float tol, unsigned int MaxIters)
{
  ...
  for( iter = 1; iter <= MaxIters; iter++ )
    ...
    if( fu <= fx )
      ...
      if( fu <= fw || itk::Math::FloatAlmostEqual(w , x) )
        {
        v = w;
        w = u;
        fv = fw;
      ...
      else if( fu <= fv || itk::Math::FloatAlmostEqual(v , x) || v == w )
        {
        v = u;
        fv = fu;
        }
