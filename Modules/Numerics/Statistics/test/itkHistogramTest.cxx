...
int itkHistogramTest(int, char* [] )
{
  ...
  if (itk::Math::NotEqualsComparison((lowerBound[0] + interval * 31), histogram->GetBinMin(0,31)))
    {
    pass = false;
    whereFail = "GetBinMin(Dimension, nthBin)";
    }

  if (itk::Math::NotEqualsComparison((lowerBound[0] + interval * 32), histogram->GetBinMax(0,31)))
    {
    pass = false;
  ...
  if (itk::Math::NotEqualsComparison((lowerBound[0] + interval * 31         ), histogram->GetBinMinFromValue(0, lowerBound[0] + interval * 31.5 ))
   || itk::Math::NotEqualsComparison((lowerBound[0]                         ), histogram->GetBinMinFromValue(0, itk::NumericTraits< float >::min()   ))
   || itk::Math::NotEqualsComparison((lowerBound[0] + interval * (size[0]-1)), histogram->GetBinMinFromValue(0, itk::NumericTraits< float >::max() )) )
    {
    pass = false;
    whereFail = "GetBinMinFromValue(Dimension, A Value Within The Nth Bin)";
    }

  if (itk::Math::NotEqualsComparison((lowerBound[0] + interval * 32         ), histogram->GetBinMaxFromValue(0, lowerBound[0] + interval * 31.5 ))
   || itk::Math::NotEqualsComparison((lowerBound[0] + interval              ), histogram->GetBinMaxFromValue(0, itk::NumericTraits< float >::min()   ))
   || itk::Math::NotEqualsComparison((upperBound[0]                         ), histogram->GetBinMaxFromValue(0, itk::NumericTraits< float >::max() )) )
    {
    pass = false;
  ...
  index.Fill(31);
  if (itk::Math::NotEqualsComparison((lowerBound[0] + interval * 31), histogram->GetHistogramMinFromIndex(index)[0]))
    {
    pass = false;
    whereFail = "GetHistogramMinFromIndex(Dimension, nthBin)";
    }

  if (itk::Math::NotEqualsComparison((lowerBound[0] + interval * 32), histogram->GetHistogramMaxFromIndex(index)[0]))
    {
    pass = false;
  ...
  if( itk::Math::NotEqualsComparison(quantile1, 307.2))
    {
  ...
  if( itk::Math::NotEqualsComparison(quantile3, 716.8))
    {
  ...
  if (itk::Math::NotEqualsComparison((lowerBound[0] + interval * 31), sparseHistogram->GetBinMin(0,31)))
    {
    pass = false;
    whereFail = "Sparse Histogram: GetBinMin(Dimension, nthBin)";
    }

  if (itk::Math::NotEqualsComparison((lowerBound[0] + interval * 32), sparseHistogram->GetBinMax(0,31)))
    {
    pass = false;
  ...
  for( unsigned kid0 = 0; kid0 < numberOfComponents; kid0++ )
    {
    if( itk::Math::NotEqualsComparison(measurement[kid0], 8) )
      {
  ...
  for( unsigned kid1 = 0; kid1 < numberOfComponents; kid1++ )
    {
    if( itk::Math::NotEqualsComparison(measurement[kid1], 8) )
      {
      std::cerr << "GetMeasurementVector() for index = ";
      std::cerr << index << std::endl;
      pass = false;
  ...
  for( unsigned kid2 = 0; kid2 < numberOfComponents; kid2++ )
    {
    if( itk::Math::NotEqualsComparison(measurement[kid2], 8) )
      {
