...
::SetMean(double mean)
{
  ...
  if ( m_Parameters.GetSize() > 0 )
    {
    if ( !itk::Math::FloatAlmostEqual(m_Parameters[0] , mean) )
      {
      modified = true;
      }
    }
...
::SetVariance(double variance)
{
  ...
  if ( m_Parameters.GetSize() > 1 )
    {
    if ( !itk::Math::FloatAlmostEqual(m_Parameters[1] , variance) )
      {
      modified = true;
      }
    }
...
::InverseCDF(double p, double mean, double variance)
{
  ...
  if ( itk::Math::FloatAlmostEqual(x , itk::NumericTraits< double >::NonpositiveMin()) )
    {
    return x;
    }
  else if ( itk::Math::FloatAlmostEqual(x , itk::NumericTraits< double >::max()) )
    {
    return x;
    }
  else
