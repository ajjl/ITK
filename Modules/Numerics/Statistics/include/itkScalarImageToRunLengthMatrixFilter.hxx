    ...
    for( neighborIt.GoToBegin(); !neighborIt.IsAtEnd(); ++neighborIt )
      ...
      while ( inputImage->GetRequestedRegion().IsInside(index) )
        ...
        if ( pixelIntensity >= centerBinMin
            && ( pixelIntensity < centerBinMax || ( itk::Math::FloatAlmostEqual(pixelIntensity , centerBinMax) && centerBinMax == lastBinMax ) ) )
          {
...
{
  if( !itk::Math::FloatAlmostEqual(this->m_MinDistance , min) || this->m_MaxDistance != max )
    {
